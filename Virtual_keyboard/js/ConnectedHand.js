  ...
  ConnectedHand.prototype.onTouch = function(cube){
    for(var i =0;i<5;i++){
      var finger = this.fingers[i];
      for(var j=0;j<4;j++){
        var connection = finger.connections[j];
        if(connection){
          /*
          var dx = (cube.position.x-connection.position.x);
          var dy = (cube.position.y-connection.position.y);
          var dz = (cube.position.z-connection.position.z);
          ...
          if(cube.geometry.boundingBox.containsPoint(connection.position)){
            cube.pressed = true;
          }
        }
      }
  ...
  ConnectedHand.prototype.createMesh = function(){
    ...
    this.createFingers( jointMesh , connectionMesh );
    this.createPalm( jointMesh , connectionMesh , centerMesh );
  }
